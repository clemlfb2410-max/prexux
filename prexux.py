...
def ask_ai():
    ...
    if "je m'appelle" in low_input or "mon nom est" in low_input:
        ...
        reply = (
            f"Tu es {name}."
            if name
            else "Je ne connais pas encore ton prénom. Dis-moi 'Je m'appelle...' !"
        )
...
def clear_memory():
    ...
    return jsonify(
        {"message": "PREXUS : Mémoire réinitialisée.", "tag": "MEM: RESET"}
    )
...
def get_ram():
    if not psutil:
        return jsonify(
            {
                "message": "RAM : Installe psutil ('pip install psutil') pour voir les infos.",
                "tag": "RAM: N/A",
            }
        )
    ...
    return jsonify(
        {
            "message": f"RAM : {ram.percent}% ({used_gb} GB / {total_gb} GB)",
            "tag": f"RAM: {ram.percent}%",
        }
    )
...
def get_cpu():
    if not psutil:
        return jsonify(
            {
                "message": "CPU : Installe psutil ('pip install psutil') pour voir les infos.",
                "tag": "CPU: N/A",
            }
        )
    ...
    return jsonify(
        {
            "message": f"CPU : Charge à {cpu}% ({psutil.cpu_count()} cœurs).",
            "tag": f"CPU: {cpu}%",
        }
    )
...
def get_sys():
    return jsonify(
        {
            "message": f"SYSTEME : {platform.node()} sous {platform.system()} {platform.release()}.",
            "tag": "OS: OK",
        }
    )
